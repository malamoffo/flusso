...
import { X, Trash2, Download, Copy, Check, Search, Terminal } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { cn } from '../lib/utils';
import { consoleInterceptor, LogEntry, LogType } from '../utils/consoleInterceptor';
import { format } from 'date-fns';

export function BrowserLogsModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleCopy = () => {
    const text = filteredLogs.map(log => `[${format(log.timestamp, 'HH:mm:ss')}] [${log.type.toUpperCase()}] ${log.message}`).join('\n');
  ...
  const handleDownload = () => {
    const text = logs.map(log => `[${format(log.timestamp, 'HH:mm:ss')}] [${log.type.toUpperCase()}] ${log.message}`).join('\n');
    const blob = new Blob([text], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `flusso-logs-${format(new Date(), 'yyyy-MM-dd-HH-mm')}.txt`;
          ...
          >
            ...
            <div className="p-6 border-b border-gray-800 flex items-center justify-between bg-gray-950/80 backdrop-blur-xl shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-white leading-none">Console Logs</h2>
                  <p className="text-[10px] text-gray-500 uppercase tracking-widest mt-1">System Debug Output</p>
                </div>
              </div>
              <button 
                onClick={onClose}
            ...
            <div className="p-4 border-b border-gray-800 flex flex-col gap-4 bg-gray-900/30 shrink-0">
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-gray-900 border border-gray-800 rounded-xl text-sm text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
                />
              </div>
              
              <div className="flex items-center justify-between gap-2 overflow-x-auto no-scrollbar">
                <div className="flex gap-1">
                  {(['all', 'log', 'info', 'warn', 'error'] as const).map((t) => (
                    <button
                      key={t}
                      onClick={() => setFilter(t)}
                      className={cn(
                ...
                <div className="flex gap-1 shrink-0">
                  ...
                  <button 
                    onClick={() => consoleInterceptor.clearLogs()}
            ...
            <div 
              ref={scrollRef}
              className="flex-1 overflow-y-auto p-4 font-mono text-[11px] leading-relaxed selection:bg-indigo-500/30 custom-scrollbar"
            >
              {filteredLogs.length === 0 ? (
                ...
                <div className="space-y-1">
                  {filteredLogs.map((log, index) => (
                    <div key={index} className="flex gap-3 group border-b border-gray-900/50 pb-1">
                      <span className="text-gray-600 shrink-0 select-none">
                        {format(log.timestamp, 'HH:mm:ss')}
                      </span>
                      <span className={cn(
                        "font-bold shrink-0 w-12 text-center select-none opacity-80",
